    ...
    <>
      <div className="flex h-[100dvh] bg-slate-50 font-sans text-slate-800 overflow-hidden">
        ...
        <aside className={`fixed inset-y-0 left-0 w-64 bg-[#e0e5ec] dark:bg-[#23262e] border-r border-white/50 dark:border-white/5 flex flex-col shrink-0 z-[90] h-[100dvh] shadow-2xl transform transition-transform duration-300 ease-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          ...
          <nav onClick={() => setIsSidebarOpen(false)} className="flex-1 overflow-y-auto px-3 py-3 md:py-4 space-y-1 hide-scrollbar">
            {profile?.role === "ADMIN" && (
              <>
          ...
          <div className="p-4 border-t border-slate-100 shrink-0 space-y-2">
            {deferredPrompt && (
              <button
                ...
                className="w-full flex items-center justify-center gap-2 bg-emerald-600/10 text-emerald-600 border border-emerald-500/30 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-600 hover:text-white transition-all"
              >
                <ScanBarcode className="w-4 h-4" /> Pasang Aplikasi (PWA)
              </button>
            )}

            <div className="flex items-center gap-3 bg-slate-50 p-3 rounded-xl border border-slate-100 mb-1 overflow-hidden">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1 mt-0.5">
                  <span
                    className={`px-1.5 py-0.5 rounded text-[8px] font-bold uppercase tracking-widest ${profile?.role === "ADMIN" ? "bg-blue-500/20 text-blue-600" : profile?.role === "AUDIT" ? "bg-amber-500/20 text-amber-600" : "bg-emerald-500/20 text-emerald-600"}`}
                  >
                    {profile?.role}
                  </span>
                </div>
            ...
            <button
              onClick={handleAppLogout}
              className="w-full flex items-center justify-center gap-2 text-[10px] font-bold text-rose-500 hover:bg-rose-500/10 py-2.5 rounded-xl transition-colors uppercase tracking-widest border border-slate-100"
            >
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                              ...
                              <input
                                ...
                                required
                              />
                              <div className="mt-2 flex items-start gap-2 rounded-2xl bg-blue-50/70 dark:bg-blue-950/40 border border-blue-100 dark:border-blue-900 px-3 py-2">
                                <Sparkles className="w-3.5 h-3.5 text-blue-500 dark:text-blue-400 mt-0.5 shrink-0" />
                                <div className="min-w-0">
                                  <p className="text-[8px] font-black text-blue-500 dark:text-blue-400 uppercase tracking-widest leading-none">
                                    Nama Produk Tersimpan
                                  </p>
                                  <p className="text-[11px] font-black text-slate-700 dark:text-slate-200 mt-1 break-words leading-snug">
                                    {buildProductName() || "—"}
                                  </p>
                                </div>
                              </div>
              ...
              (profile?.role === "AUDIT" || profile?.role === "ADMIN") && (
                <div className="flex-1 flex flex-col bg-white md:m-4 md:rounded shadow-sm border-t md:border border-slate-200 overflow-hidden">
                  <div className="p-4 border-b border-slate-200 bg-slate-100 flex flex-col gap-4 justify-between shrink-0">
                    ...
                    {!searchTerm && (
                      <div className="flex items-center gap-2 overflow-x-auto py-1 no-scrollbar text-[10px] font-bold uppercase tracking-widest">
                        <button
                          onClick={() => setDrillPath([])}
                          className={`shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-lg border transition-all ${drillPath.length === 0 ? "bg-emerald-600 border-emerald-600 text-white shadow-[0_0_16px_2px_rgba(16,185,129,0.55)]" : "bg-slate-50 border-transparent text-slate-500 shadow-[3px_3px_7px_#b8c2d0,-3px_-3px_7px_#ffffff] dark:shadow-[3px_3px_7px_#1c1e24,-3px_-3px_7px_#34373f] hover:text-slate-600"}`}
                        >
                          <LayoutList className="w-3 h-3" />
                          SEMUA KATEGORI
                        </button>
                  ...
                  <div className="w-full lg:w-[350px] bg-slate-50 overflow-y-auto shrink-0 shadow-inner flex flex-col h-full border-l border-slate-200 relative">
                      ...
                      <div className="p-4 bg-slate-50 border-b border-slate-200/60 sticky top-0 z-10 text-left">
                        <div className="flex gap-1 bg-slate-50 p-1 rounded-2xl overflow-x-auto no-scrollbar shadow-[inset_3px_3px_7px_#b8c2d0,inset_-3px_-3px_7px_#ffffff] dark:shadow-[inset_3px_3px_7px_#1c1e24,inset_-3px_-3px_7px_#34373f]">
                          <button
                            onClick={() => setAuditSidebarTab("incoming")}
                            className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl transition-all font-black text-[9px] uppercase tracking-widest whitespace-nowrap ${auditSidebarTab === "incoming" ? "bg-slate-50 text-emerald-600 shadow-[0_0_14px_1px_rgba(16,185,129,0.45),3px_3px_7px_#b8c2d0,-3px_-3px_7px_#ffffff] dark:shadow-[0_0_14px_1px_rgba(16,185,129,0.45),3px_3px_7px_#1c1e24,-3px_-3px_7px_#34373f]" : "bg-transparent text-slate-400 hover:text-slate-600"}`}
                          >
                            <Plus className="w-3.5 h-3.5" /> Stok Masuk
                          </button>
                          <button
                            onClick={() => setAuditSidebarTab("disposal")}
                            className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl transition-all font-black text-[9px] uppercase tracking-widest whitespace-nowrap ${auditSidebarTab === "disposal" ? "bg-slate-50 text-red-600 shadow-[0_0_14px_1px_rgba(239,68,68,0.45),3px_3px_7px_#b8c2d0,-3px_-3px_7px_#ffffff] dark:shadow-[0_0_14px_1px_rgba(239,68,68,0.45),3px_3px_7px_#1c1e24,-3px_-3px_7px_#34373f]" : "bg-transparent text-slate-400 hover:text-slate-600"}`}
                          >
                            <Trash2 className="w-3.5 h-3.5" /> Pemusnahan
                          </button>
                          <button
                            onClick={() => setAuditSidebarTab("transfer")}
                            className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl transition-all font-black text-[9px] uppercase tracking-widest whitespace-nowrap ${auditSidebarTab === "transfer" ? "bg-slate-50 text-blue-600 shadow-[0_0_14px_1px_rgba(59,130,246,0.45),3px_3px_7px_#b8c2d0,-3px_-3px_7px_#ffffff] dark:shadow-[0_0_14px_1px_rgba(59,130,246,0.45),3px_3px_7px_#1c1e24,-3px_-3px_7px_#34373f]" : "bg-transparent text-slate-400 hover:text-slate-600"}`}
                          >
                            <ArrowRightLeft className="w-3.5 h-3.5" /> Antar Cabang
                          </button>
                          <button
                            onClick={() => setAuditSidebarTab("logs")}
                            className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl transition-all font-black text-[9px] uppercase tracking-widest whitespace-nowrap ${auditSidebarTab === "logs" ? "bg-slate-50 text-amber-600 shadow-[0_0_14px_1px_rgba(245,158,11,0.45),3px_3px_7px_#b8c2d0,-3px_-3px_7px_#ffffff] dark:shadow-[0_0_14px_1px_rgba(245,158,11,0.45),3px_3px_7px_#1c1e24,-3px_-3px_7px_#34373f]" : "bg-transparent text-slate-400 hover:text-slate-600"}`}
                          >
                  ...
                  <div className="flex gap-1 bg-slate-50 p-1 rounded-2xl shadow-[inset_3px_3px_7px_#b8c2d0,inset_-3px_-3px_7px_#ffffff] dark:shadow-[inset_3px_3px_7px_#1c1e24,inset_-3px_-3px_7px_#34373f]">
                    {[
                      ...
                      <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl transition-all font-black text-[9px] uppercase tracking-widest ${posSubView === tab.id ? "bg-slate-50 text-blue-600 shadow-[0_0_14px_1px_rgba(59,130,246,0.45),3px_3px_7px_#b8c2d0,-3px_-3px_7px_#ffffff] dark:shadow-[0_0_14px_1px_rgba(59,130,246,0.45),3px_3px_7px_#1c1e24,-3px_-3px_7px_#34373f]" : "bg-transparent text-slate-400 hover:text-slate-600"}`}
                      >
                          ...
                          <button
                            ...
                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${
                              posSelectedCategory === "Semua"
                                ? "bg-slate-900 border border-slate-950 text-white shadow-[0_0_16px_2px_rgba(15,23,42,0.4)]"
                                : "bg-slate-50 text-slate-500 shadow-[3px_3px_7px_#b8c2d0,-3px_-3px_7px_#ffffff] dark:shadow-[3px_3px_7px_#1c1e24,-3px_-3px_7px_#34373f] hover:text-slate-600"
                            }`}
                          >
                              ...
                              <button
                                ...
                                className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest whitespace-nowrap transition-all ${
                                  posSelectedCategory === cat
                                    ? "bg-blue-600 border border-blue-700 text-white shadow-[0_0_16px_2px_rgba(59,130,246,0.5)]"
                                    : "bg-slate-50 text-slate-500 shadow-[3px_3px_7px_#b8c2d0,-3px_-3px_7px_#ffffff] dark:shadow-[3px_3px_7px_#1c1e24,-3px_-3px_7px_#34373f] hover:text-slate-600"
                                }`}
                              >
                                <span>{cat}</span>
                                  ...
                                  >
                                    ...
                                    <div className="flex items-center gap-1.5 shrink-0 scale-90">
                                      <button
                                        ...
                                        className="w-7 h-7 bg-slate-50 text-slate-400 hover:text-red-600 rounded-lg transition-all active:scale-90 flex items-center justify-center font-bold shadow-[2px_2px_5px_#b8c2d0,-2px_-2px_5px_#ffffff] dark:shadow-[2px_2px_5px_#1c1e24,-2px_-2px_5px_#34373f] active:shadow-[inset_2px_2px_4px_#b8c2d0,inset_-2px_-2px_4px_#ffffff] dark:active:shadow-[inset_2px_2px_4px_#1c1e24,inset_-2px_-2px_4px_#34373f]"
                                      >
                                      ...
                                      <button
                                        onClick={() => updateCartQty(item.product.id, 1)}
                                        className="w-7 h-7 bg-slate-50 text-slate-400 hover:text-emerald-600 rounded-lg transition-all active:scale-90 flex items-center justify-center font-bold shadow-[2px_2px_5px_#b8c2d0,-2px_-2px_5px_#ffffff] dark:shadow-[2px_2px_5px_#1c1e24,-2px_-2px_5px_#34373f] active:shadow-[inset_2px_2px_4px_#b8c2d0,inset_-2px_-2px_4px_#ffffff] dark:active:shadow-[inset_2px_2px_4px_#1c1e24,inset_-2px_-2px_4px_#34373f]"
                                      >
